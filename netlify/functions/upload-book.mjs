...
const allowed=new Set([...books.flatMap(book=>langs.map(lang=>`${book}/${lang}.pdf`)),'almoravides-research/ar.pdf']);
...
export default async (req)=>{
  ...
  if(action==='chunk'){
    const uploadId=url.searchParams.get('uploadId')||'',part=Number(url.searchParams.get('part')),total=Number(url.searchParams.get('total'));
  ...
  if(action==='finalize'){
    const uploadId=url.searchParams.get('uploadId')||'',total=Number(url.searchParams.get('total'));
    ...
    const chunks=getStore('ebook-upload-chunks'),parts=[];let size=0;
    for(let part=0;part<total;part++){const chunk=await chunks.get(`${uploadId}/${part}`,{type:'arrayBuffer'});if(!chunk)return new Response(`Segment ${part+1}/${total} manquant.`,{status:409});size+=chunk.byteLength;if(size>MAX_PDF_SIZE)return new Response('Le PDF dépasse la limite de 50 Mo.',{status:413});parts.push(new Uint8Array(chunk))}
    const pdf=new Uint8Array(size);let offset=0;for(const part of parts){pdf.set(part,offset);offset+=part.byteLength}
