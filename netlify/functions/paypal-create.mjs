...
export default async (req)=>{
  ...
  try{
    ...
    const returnUrl=`${base}/acheter.html?book=${encodeURIComponent(book)}&lang=${encodeURIComponent(lang)}`;
    const r=await fetch(`${PAYPAL_API}/v2/checkout/orders`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json','PayPal-Request-Id':crypto.randomUUID()},body:JSON.stringify({intent:'CAPTURE',purchase_units:[{custom_id:`${book}:${lang}`,description:`Bawaba PDF - ${book} (${lang})`,amount:{currency_code:'EUR',value:'4.99'}}],application_context:{shipping_preference:'NO_SHIPPING',return_url:returnUrl,cancel_url:returnUrl,user_action:'PAY_NOW'}})});
